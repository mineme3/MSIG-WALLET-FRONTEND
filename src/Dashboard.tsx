        ...
        <div className="space-y-4">
          {txs.length === 0 ? (
            ...
            txs.slice().reverse().map((tx) => (
              <div
                key={tx.index}
                className={`group relative bg-gray-900 p-6 rounded-3xl border transition-all duration-300 ${
                  tx.executed 
              ...
              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-3">
                      <p className="text-lg font-bold text-white leading-none">{tx.numConfirmations.toString()}</p>
                      <div className="flex-1 h-1 bg-gray-800 rounded-full overflow-hidden">
                        <div 
                          className={`h-full rounded-full transition-all duration-1000 ${tx.executed ? 'bg-success' : 'bg-primary'}`}
                          style={{ width: `${Math.min((Number(tx.numConfirmations) / 2) * 100, 100)}%` }}
                        ></div>
                      </div>
                    </div>
                  </div>
                ...
                <div className="flex flex-wrap gap-2 pt-4 border-t border-gray-800/50">
                  ...
                  <button
                    disabled={tx.executed || tx.numConfirmations < 2 || loadingTx === tx.index}
